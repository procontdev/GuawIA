-- 1. Extender tabla guaw.orders para trazabilidad de delivery
ALTER TABLE guaw.orders 
ADD COLUMN IF NOT EXISTS delivery_zone_id UUID REFERENCES guaw.delivery_zones(id),
...
ADD COLUMN IF NOT EXISTS delivery_resolution_method TEXT,
ADD COLUMN IF NOT EXISTS delivery_resolution_detail JSONB;

COMMENT ON COLUMN guaw.orders.coverage_status IS 'covered | out_of_coverage | insufficient_data';

-- 2. Asegurar extensión unaccent para búsquedas robustas
CREATE EXTENSION IF NOT EXISTS unaccent;

-- 3. Función de Resolución de Delivery Scoring V1
...
DECLARE
    ...
    v_keyword TEXT;
    v_current_score INT;
    v_coverage_status TEXT := 'insufficient_data';
...
BEGIN
    ...
    LOOP
        ...
        -- Match de Distrito (+50)
        -- Compara contra el distrito de la zona
        IF v_norm_district = lower(unaccent(v_zone.district)) THEN
    ...
    -- Determinación de status de cobertura según umbrales propuestos
    IF v_best_score >= 70 THEN
        v_coverage_status := 'covered';
    ELSIF v_best_score >= 40 THEN
        v_coverage_status := 'insufficient_data'; -- Requiere intervención o más info
    ELSE
